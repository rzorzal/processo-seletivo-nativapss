...
var generateRoutes = function(model, modelName, sequelize, models){
    ...
    router.get("/:ModelId(-?\\d+)", async function(req, res){
        ...
        if(!result){
            res.json({success: false, message: "Model with id " + req.params.ModelId + " not found!"});
            return;
        }
    ...
    router.get("/:ModelId(-?\\d+)/:ModelName", async function(req, res){
        ...
        res.json({success: false, message: "The " + req.params.ModelName + " models is not associated with " + modelName});
    });
